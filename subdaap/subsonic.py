...
class Connection(libsonic.Connection):
    """
    Wraps `libsonic.Connection` to parse URL for host and port. It also adds
    name and index as property.
    """
...
class SubSonicProvider(Provider):
    ...
    def __init__(self, db, connection, artwork_cache, item_cache):
        ...
        self.server = Server(manager=RevisionManager(), db=self.db)

        self.setup_library()

    def benchmark(self, runs=10):
        """
        Benchmark the speed of encoding of items. This gives an idea of the
        performance. Useful for testing
        """
        ...
        for i in xrange(runs):
            self.get_items(1, 1, 1, 0).encode()

        return (time.time() - start) / runs

    def setup_library(self):
        self.update_event = gevent.event.Event()

        # Verify structure
        try:
            self.server.databases[1]
        except KeyError:
            self.create_library()

        # Expose database and container
        self.database = self.server.databases[1]
        self.container = VirtualContainer(manager=self.server.manager, db=self.db, id=1, name="All Music")
        self.container.is_base = True

        self.server.add_database(self.database)
        self.database.add_container(self.container)
    ...
    def create_library(self):
        logger.info("Database not initilized, tables created.")

        # Initial structure
        library = database.Library(id=1, name="My Library")
    ...
    def wait_for_update(self):
        ...
        return self.server.manager.revision
